...
async function commitAutoMessageTurn(session) {
  ...
  return { seller_text: sellerText, reply: reply?.reply ?? null, session };
}
...
function buildHintGenerationSystemPrompt(session, snapshot, strategy = 'exploit', recentOpeners = [], uiStrategy = null) {
  ...
  const lines = [
    ...
    ``,
    `## Session signal`,
    `Signal type: ${session.sde_card?.signal_type || 'unknown'}`,
    `Heat: ${session.sde_card?.heat || 'unknown'}`,
    `Outreach window: ${session.sde_card?.outreach_window || 'unknown'}`,
    `Probable pain: ${session.sde_card?.probable_pain || ''}`,
    `Signal context: ${session.sde_card?.rendered_text || session.sde_card?.what_happened || ''}`,
    ``,
